...
核心保证：MQTT 协议限制同一 ClientID 只能有一个活跃连接。配合 Broker 认证（username/password 或 TLS），ClientID 不可被冒充。服务端通过 Broker 注入的消息属性获取发布者的 ClientID，不依赖客户端 payload。

## 为什么不用 JWT

| | JWT | Session |
|---|---|---|
| 每请求额外带宽 | ~350 字节 | 0（ClientID 由 Broker 注入，不在帧中） |
...
type Context struct {
    ...
    ClientID    string    // 发布者标识（由 Broker 注入，不在帧中传输）
...
2. 客户端发送请求:
   帧中只有 cmd + payload（不携带 ClientID）

3. 服务端验证:
   - Broker 通过消息属性注入发布者的 ClientID
   - 服务端从 transport properties 中提取 ClientID
